...
Because a bitset stores integers in some finite set, it can be returned as an integer vector by using `Bitset$to_vector()`. However, this is a slow and expensive operation, as data must be copied into a new vector which is returned to R. If your model's dynamics require the frequent returning of integer vectors for computation and updating, an `individual::IntegerVector` object will be more appropriate, or if the variable's range is unbounded, or can take negative values. However, for most discrete variables with a finite range of values, and especially those which mirror compartments in mathematical models, bitset operations and `individual::CategoricalVariable` (which uses bitsets internally) should be preferred. Taking advantage of bitset operations can help make processes in "individual" much faster.
