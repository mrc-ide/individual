...
ResizeableDoubleVariable <- R6Class(
  ...
  public = list(
    ...
    get_values = function(index = NULL) {
      if (is.null(index)) {
        return(resizeable_double_variable_get_values(self$.variable))
      } else {
        if (inherits(index, 'Bitset')) {
          return(resizeable_double_variable_get_values_at_index(self$.variable, index$.bitset))
        } else {
          stopifnot(all(is.finite(index)))
          stopifnot(all(index > 0))
          return(resizeable_double_variable_get_values_at_index_vector(self$.variable, index))
        }
      }
    },
    ...
    get_index_of = function(a, b) {
      stopifnot(a < b)
      return(Bitset$new(from = resizeable_double_variable_get_index_of_range(self$.variable, a, b)))      
    },
    ...
    get_size_of = function(a, b) {
      stopifnot(a < b)
      return(resizeable_double_variable_get_size_of_range(self$.variable, a, b))
    },
