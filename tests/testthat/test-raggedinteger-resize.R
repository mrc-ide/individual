test_that("extending RaggedInteger returns the new values", {
  size <- 10
  vals <- as.list(seq_len(size))
  x <- RaggedInteger$new(vals)
...
test_that("shrinking RaggedInteger removes values (bitset)", {
  size <- 10
  vals <- as.list(seq_len(size))
  x <- RaggedInteger$new(vals)
...
test_that("shrinking RaggedInteger removes values (vector)", {
  size <- 10
  vals <- as.list(seq_len(size))
  x <- RaggedInteger$new(vals)
  expect_equal(x$get_values(), vals)
...
test_that("RaggedInteger shrinks are combined", {
  size <- 10
  vals <- as.list(seq_len(size))
  x <- RaggedInteger$new(vals)
  x$queue_shrink(index = Bitset$new(size)$insert(1:5))
...
test_that("RaggedInteger shrinks are applied before extentions", {
  size <- 10
  vals <- as.list(seq_len(size))
  x <- RaggedInteger$new(vals)
  x$queue_shrink(index = 5:10)
