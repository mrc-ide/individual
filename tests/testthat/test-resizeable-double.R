test_that("Creating ResizeableDoubleVariables errors with bad input", {
  expect_error(ResizeableDoubleVariable$new(NULL))
  expect_error(ResizeableDoubleVariable$new(NA))
  expect_error(ResizeableDoubleVariable$new("1"))
})

test_that("ResizeableDoubleVariable get values returns correct values without index", {
  size <- 10
...
test_that("ResizeableDoubleVariable get values returns correct values with vector index", {
  size <- 10
  variable <- ResizeableDoubleVariable$new(seq_len(size))
...
test_that("ResizeableDoubleVariable get values returns correct values with bitset index", {
  size <- 10
  variable <- ResizeableDoubleVariable$new(seq_len(size) + 10)
  expect_equal(variable$get_values(Bitset$new(size = size)$insert(5:10)), 15:20)
})

test_that("ResizeableDoubleVariable get values fails with incorrect index", {
  variable <- ResizeableDoubleVariable$new(initial_values = 1:100)
  b <- Bitset$new(1000)
...
test_that("ResizeableDoubleVariable get index of bounds [a,b] works correctly", {
  variable <- ResizeableDoubleVariable$new(5:10)
...
test_that("ResizeableDoubleVariable get index of bounds [a,b] fails with incorrect bounds", {
  variable <- ResizeableDoubleVariable$new(5:10)
...
test_that("ResizeableDoubleVariable get size of set in bounds [a,b] works correctly", {
  variable <- ResizeableDoubleVariable$new(-10:10)
...
test_that("ResizeableDoubleVariable get size of set in bounds [a,b] fails with incorrect input", {
  variable <- ResizeableDoubleVariable$new(-10:10)
